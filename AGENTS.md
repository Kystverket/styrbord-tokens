...
### TypeScript / JS color tokens

In addition to CSS, the build produces `dist/color-tokens.js` and `dist/color-tokens.d.ts` via `scripts/build-ts-colors.mjs`. This script runs automatically as part of `npm run build` (after the designsystemet CLI step) and parses the generated CSS to extract hex values.

The module is exported under the `./colors` subpath in `package.json` exports:

```ts
import { colors } from '@kystverket/styrbord-tokens/colors';
colors.light.primary.baseDefault  // "#000667"
colors.dark.accent.baseDefault    // hex value for dark mode
```

The output file is intentionally named `color-tokens.*` (not `colors.*`) to avoid overwriting `dist/colors.d.ts`, which is generated by the designsystemet CLI as a type augmentation for `@digdir/designsystemet-types`.

### Release process

Releases are managed by Release Please (`release-please.yml`). When a PR is merged to `main`, release-please either creates/updates a release PR or — if it just created a GitHub release — runs the `publish` job in the same workflow to publish the package to both GitHub Packages and npmjs.org using OIDC (no token needed for npmjs).

Note: publishing runs in the same workflow as release-please (not a separate `release: published` trigger) because GitHub does not fire workflow events for releases created by `GITHUB_TOKEN`.
